...
module.exports = config => {
  const { user = false, token = false, url = false, project = [], status = [] } = config
  return async choice => {
    ...
    if(project.length > 0) opts.push(`project in (${project.join(', ')})`)
    if(status.length > 0) opts.push( ` AND status in (${statuses.join(', ')})`)
